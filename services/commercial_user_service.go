...
func (as *UserService) GetAllDcddUsers(limit, offset int) ([]model.DcddUser, int, error) {
	return as.Repository.FetchAllDcddUsers(limit, offset)
}

func (as *UserService) GetAllActiveDcddUsers(limit, offset int) ([]model.DcddUser, int, error) {
...
func (vs *UserService) GetDcddUserTotals(fromDate, toDate *time.Time, school_id, eccd_id, dzongkhag_id *uuid.UUID, category *string) (totalAll int, totalActive int, totalNew int, totalInActive int, err error) {
    return vs.Repository.GetDcddUserTotals(fromDate, toDate, school_id, eccd_id, dzongkhag_id, category)
}
